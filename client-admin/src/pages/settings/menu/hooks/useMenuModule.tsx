...
const getStatusTag = (status: 0 | 1) => {
  const color = status === 1 ? 'green' : 'default';
...
export const useMenuModule = () => {
  ...
  const openAdd = useCallback((parentId?: number) => {
    setMode('add');
    const hasParent = typeof parentId === 'number' && parentId > 0;
    setInfo(hasParent ? ({ id: '', parentId, menuName: '', menuType: 2, status: 1 } as MenuItem) : null);
    setVisible(true);
  }, []);
